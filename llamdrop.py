...
def _lookup_prompt_format(models_json, filename):
    """Look up prompt_format from models.json by matching filename."""
    try:
        for m in models_json.get("models", []):
            for variant in m.get("variants", {}).values():
                if variant.get("filename", "") == filename:
                    return m.get("prompt_format", "chatml")
    except Exception:
        pass
    return "chatml"
