...
def show_hf_search(device_profile):
    """
    HuggingFace live search screen.
    Fixes:
      - Loop so user can search multiple times without going back to main menu
      - Dedicated [S] search again and [B] back options after results
      - Shows result count before opening browser
    """
    while True:
        os.system("clear")
        print_banner()
        ...
        print(c(CYAN,   "  [B] Back to main menu\n"))

        try:
            raw = input(f"  {t('search_prompt')}").strip()
        except (EOFError, KeyboardInterrupt):
            return None

        # Dedicated back option
        ...
        if not results:
            print(f"\n  {t('search_none')}")
            print(f"\n  [S] Search again   [B] Back to main menu")
            try:
                choice = input("\n  > ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                return None
            if choice in ("b", "back"):
                return None
            # anything else (s, enter, etc.) → loop back to search
            continue

        print(f"\n  Found {len(results)} compatible model(s). Opening browser...\n")
        time.sleep(0.6)
        ...
        # User pressed Q/Esc in browser — ask what to do next
        os.system("clear")
        print_banner()
        print(c(BOLD, f"  {t('menu_search')}\n"))
        print(f"  Last search: \"{query}\"  ({len(results)} results)\n")
        ...
        if choice in ("b", "back"):
            return None
        # S or anything else → loop back to new search
