...
def smart_pick_variant(model):
    ...
    # Q6_K / Q8_0 added (Bug fix: were missing, causing downloader to silently
    # pick a lower quant even when the user selected a Q6 model in the browser).
    # IQ variants sit between Q3 and Q2 — CPU-only but better than Q2_K.
    pref_order = [
        "Q8_0",
        "Q6_K",
        "Q5_K_M", "Q5_K_S", "Q5_K",
...
def draw_progress(downloaded, total, speed_bps=0, elapsed_secs=0):
    if total > 0:
        ...
        # ETA calculation
        eta_str = ""
        if speed_bps > 0 and downloaded < total:
            remaining_bytes = total - downloaded
            eta_secs = int(remaining_bytes / speed_bps)
            if eta_secs < 60:
                eta_str = f"  ETA {eta_secs}s"
            else:
                eta_str = f"  ETA {eta_secs // 60}m{eta_secs % 60:02d}s"
        line = f"\r  [{bar}] {pct}%  {dl_str}/{tot_str}  {spd_str}{eta_str}    "
    else:
        dl_str = format_size(downloaded)
        ...
        with open(dest_path, mode) as f:
            while True:
                ...
                if now - last_update >= 0.5:
                    ...
                    draw_progress(downloaded, total_size, speed, elapsed)
                    last_update = now
