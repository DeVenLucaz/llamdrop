    ...
    try:
        while True:
            ...
            if user_input.lower() in ("/quit", "/exit", "/q"):
                ...
                live_ram = get_available_ram_gb()
                print(f"  {ram_status_line(live_ram)}")
                continue
...
def _export_chat(history, model_name):
    ...
    # Sanitise model name for use in filename — keep alphanumeric, dots, dashes
    import re as _re
    safe_model = _re.sub(r"[^a-zA-Z0-9.\-]", "-", model_name)[:30].strip("-").lower()
    filename  = f"llamdrop-{safe_model}-{timestamp}.md"
